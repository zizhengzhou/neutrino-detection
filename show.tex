...
\section{Reactor Neutrino CENvS Signal}
\begin{frame}{Reactor Neutrino CENvS Signal: Target comparison}
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{figure}
            \centering
            \includegraphics[width=.8\textwidth]{fig theory/CENvS/Daya_Bay_calc_10m.png}
            \caption{Si target}
        \end{figure}
        \column{0.5\textwidth}
        \begin{figure}
            \centering
            \includegraphics[width=.8\textwidth]{fig theory/CENvS/Daya_Bay_calc_10m——Xe.png}
            \caption{Xe target}
        \end{figure}
    \end{columns}
    From 10eV to 1000eV, the event rate per energy increases about 1 order for Si, while it increases about 5 orders for Xe. The difference is due to the mass of the target. 
    We suggest to use heavy particles like Ge(5.4 Si signal), GaAs(5.1 Si signal) \dots as the target.
\end{frame}

\begin{frame}{Neutrino Flux usage}
    \begin{columns}
        \column{0.5\textwidth}
        \begin{figure}
            \centering
            \includegraphics[width=\textwidth]{fig theory/CENvS/flux usage.png}
        \end{figure}

        \column{0.4\textwidth}
        For a specific nuclear recoil energy($T$) detection threshold, 
        only neutrinos with sufficient energy have nonzero cross section due to the kinematic constraint.

        \vspace{1em}

        $  E^{\rm cutoff}_\nu = (T + \sqrt{T^2 + 2T M_{\rm target}}) / 2$

        \vspace{2em}

        Total neutrinos contributed to the CENvS signal is:
        $\Phi\equiv\int_{E^{\rm cutoff}_\nu}^\infty \phi_\nu(E_\nu) dE_\nu$.

        \vspace{1em}

        A lower detection threshold can cover more neutrinos.
    \end{columns}
\end{frame}
    ...
    \begin{columns}[c]
        ...
        \column{0.5\textwidth}
            ...
            \begin{itemize}
                ...
                The total background of neutrons in our ROI is about $200$ events/y/kg.
...
\begin{frame}
    \frametitle{The result of COHERENT measurement}
        ...
        \begin{figure}
            \centering
            \includegraphics[width=\textwidth]{spallation/on-off data.png}
        \end{figure}
        \column{0.7\textwidth}
        \begin{figure}
            \centering
            \includegraphics[width=.7\textwidth]{spallation/spectra.png}
        \end{figure}
        \small The analysis employs an extensive statistical procedure, cascading from background PDF calibration to a simultaneous 2D maximum likelihood fit. 
        While mathematically rigorous, such a high degree of data processing has only 3.9 $\sigma$ significance. It is hardly for us to optimize the background subtraction. Since the ROl and source in our setup are much weaker in comparison (as listed below), even achieving 3 $\sigma$ significance would be a significant challenge.
    \end{columns}
